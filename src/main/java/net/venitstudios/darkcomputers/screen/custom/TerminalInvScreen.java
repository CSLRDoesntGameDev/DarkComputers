...
public class TerminalInvScreen extends AbstractContainerScreen<TerminalInvMenu> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        int rowCount = ((y2 - y1) / 8) - 2;
        int colCount = ((x2 - x1) / 6);

        TextEditor editor = terminalMenu.blockEntity.editor;

        String rowColInfo = String.format("L %03d C %03d", editor.CurRow, editor.CurCol);
        guiGraphics.fill(x1, y1, x2, y1 + 9, 0xFFA5A5A5);
        guiGraphics.fill(x1, y2, x2, y2 - 18, 0xFFA5A5A5);
        guiGraphics.drawString(this.font, rowColInfo, x2 - (rowColInfo.length() * 6), this.topPos + 21, 0xFF404040, false);
        ...
        for (int r = 2; r < rowCount; r++) {
            ...
            if (cr < editor.FileContents.length) {
                ...
                // cursor
                int cx1 = Math.min(x2 - 8, x1 + (formatLength * 6) + 6 + (editor.CurCol * 6));
